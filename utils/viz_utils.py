...
def draw_equilateral_triangle_markov_chain_graph(P, node_labels, node_colors=None, ax=None):
    ...
    nodes = nx.draw_networkx_nodes(G, pos, node_color=node_colors, node_size=[700]*3, ax=ax)
    ...
    nx.draw_networkx_labels(G, pos, labels=labels, font_size=9, font_color='gray', ax=ax)
    ax.set_aspect('equal')
    ax.axis('off')
    
    for ann in edge_label_annotations:
        [node_i, node_j], pos, pij = ann
        ax.annotate(f"{node_i}{node_j}={pij}", 
                    xy=pos, 
                    fontsize=9,
